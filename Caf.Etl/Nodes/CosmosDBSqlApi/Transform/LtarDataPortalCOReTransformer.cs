...
namespace Caf.Etl.Nodes.CosmosDBSqlApi.Transform
{
    public class LtarDataPortalCOReTransformer
    {
            ...
            List<MeasurementV2> measurements)
        {
            ...
            IEnumerable<IGrouping<DateTime, MeasurementV2>> groups = measurements.GroupBy(d => d.DateTime.Value);
            foreach (IGrouping<DateTime, MeasurementV2> group in groups)
            {
                // TODO: Don't hardcode!  When you have time...
                // TODO: Unit conversions!
                //var at = group.SingleOrDefault(m => m.Name == "TemperatureAirTsAvg");
                ...
                Observation o = new Observation(
                    ...
                    utcOffset,
                    recordType,
                    Convert.ToDecimal(airTemp),
                    Convert.ToDecimal(windSp),
                    Convert.ToDecimal(windDir),
                    Convert.ToDecimal(rH),
                    Convert.ToDecimal(precip),
                    Convert.ToDecimal(airPress),
                    Convert.ToDecimal(par),
                    null,
                    null,
                    Convert.ToDecimal(battVolt),
                    Convert.ToDecimal(loggerTemp));
